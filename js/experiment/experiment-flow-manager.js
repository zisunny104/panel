...
class ExperimentFlowManager {
  ...
  async startExperiment(options = {}) {
    ...
    try {
      ...
      if (options.units) {
        ...
        this.loadedUnits = this.dependencies.combinationManager.getLoadedUnits();
      }
  ...
  setCurrentUnit(unitIndex) {
    ...
    Logger.debug('當前單元已設定', unitIndex);
    return true;
  }
  ...
  getProgress() {
    return {
      ...
      percentage: this.loadedUnits.length > 0 
        ? Math.round((this.completedUnits.size / this.loadedUnits.length) * 100)
        : 0
    };
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = ExperimentFlowManager;
}
