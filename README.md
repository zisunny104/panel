...
- 統一 SyncEvents 常數並支援 LOCAL 角色；修正面板實驗控制按鈕的初始化時機與行為一致性
- 改善 QR Code 生成與掃描：相機列表排序、過濾、備援重試與偵錯輸出
- 後端：新增簡易 metrics endpoint (/metrics)，加入 rate limiting 與低頻 session 驗證排程，改善心跳與連線清理邏輯
- UI/用語：統一「退出工作階段」與「斷線」用詞，翻譯並同質化 CSS 註解；調整分享/複製流程回饋
- 重構與品質提升：命名匯出一致化、事件常數化、ESLint 設定新增、程式整理與錯誤修正
